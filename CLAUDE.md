...
**Frontend state**: two React contexts wrap the whole app (`main.tsx`): `SettingsProvider` (theme + currency, persisted to `localStorage`, no backend calls) and `AuthProvider` (JWT + username in `localStorage`, wraps the auth API calls). `App.tsx` does manual view-switching via a `activeKey` string state (`dashboard`/`transactions`/`profile`/`members`/`activity`/`yearly-schedule`/`settings`/`announcements`/`my-notifications`) rendered through a `switch` — there's no router library. The `transactions` and `activity` menu items/routes are only included/rendered when `isAdmin` is true, with a `useEffect` that redirects `activeKey` back to `dashboard` if an admin logs out mid-view (or a non-admin somehow lands on one, e.g. a stale link).

**antd `Table` pagination gotcha (bit us twice already):** pass `defaultPageSize`, never `pageSize`, in a `pagination={{ ... }}` prop that isn't otherwise fully controlled. `pageSize` makes antd treat it as a controlled prop, so the size-changer dropdown's selection gets silently reset back to it on the next re-render — see the fix in `MembersList.tsx`/`TransactionsList.tsx`. (The Activity page's server-paginated table is a different, intentionally-controlled pattern — `current`/`total`/`onChange` with `showSizeChanger: false` — and isn't subject to this.)
